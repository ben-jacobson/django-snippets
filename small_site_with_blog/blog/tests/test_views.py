...
class BlogViewTests(TestCase):
    def setUp(self):
        ...
        self.entry_one = create_test_blog_entry(title="test post one", author=test_author)
        self.entry_one.publish()            # publish the first one normally. Publish will save automatically, no need for an additional save
        self.entry_two = create_test_blog_entry(title="test post two", author=test_author)
        self.entry_two.date_published = timezone.localtime() + timedelta(hours=1)  # publish the second one in tomorrow date
        self.entry_two.save()
        self.entry_three = create_test_blog_entry(title="test post three", author=test_author) # no publish 
